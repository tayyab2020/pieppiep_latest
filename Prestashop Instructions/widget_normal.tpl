                ...
                {if !empty($conversion_options)}
                    <div class="conversion-options {if !empty($pap_product_fields)}buttons{/if}">
                        {foreach from=$conversion_options item=conversion_option}
                            <span class="unit" data-conversion-factor="{$conversion_option.conversion_factor|escape:'htmlall':'UTF-8'}" data-unit-symbol="{$conversion_option.symbol|escape:'htmlall':'UTF-8'}">
                ...
                {if $field->visible eq 1}
                    <div class="unit-entry">
                        ...
                        <div class="input-wrapper">
                            <input name="unit_{$field->id_pap_dimension|intval}" style="width:80px;" class="{$field->dimension->name|escape:'htmlall':'UTF-8'} unit unit-entry form-control" type="text"/>
...
{if $is_ajax_render}
    <script>
        if (typeof window.pap_product === 'undefined') {
            window.product = {$product nofilter};
            window.pap_product = {$pap_product_json nofilter};
            window.text_total_price_suffix = '{$text_total_price_suffix|escape:'html':'UTF-8'}';
            window.wastage_render_type = '{$pap_wastage_options_render_type|escape:'html':'UTF-8'}';
            window.group_reduction = {$group_reduction|escape:'htmlall':'UTF-8'};
            window.is_ajax_render = {$is_ajax_render nofilter};
            window.pap_module_ajax_url = "{$link->getModuleLink('productareapacks', 'ajax', array())|escape:'quotes':'UTF-8' nofilter}";
        }
    </script>
{else}
    <script>
