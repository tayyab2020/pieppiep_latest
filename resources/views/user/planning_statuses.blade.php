                            ...
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                <div class="add-product-box">
                                    <div style="justify-content: flex-end;" class="add-product-header products">
                                        <h2 style="width: 100%;">{{__('text.Planning Statuses')}}</h2>

                                        <a style="margin-right: 10px;" href="{{route('add-planning-status')}}" class="btn add-newProduct-btn"><i class="fa fa-plus"></i> {{__('text.Add New Status')}}</a>
                                            ...
                                            <div class="col-sm-12">
                                                ...
                                                <table id="example" class="table table-striped table-hover products dt-responsive dataTable no-footer dtr-inline" role="grid" aria-describedby="product-table_wrapper_info" style="width: 100%;" width="100%" cellspacing="0">
                                                    ...
                                                    <tr role="row">
                                                        ...
                                                        <th class="sorting_asc" tabindex="0" aria-controls="product-table_wrapper" rowspan="1" colspan="1" style="width: 144px;" aria-sort="ascending" aria-label="Blood Group Name: activate to sort column descending">{{__('text.Agenda Background Color')}}</th>
                                                    ...
                                                    @foreach($statuses as $item)
                                                        <tr role="row" class="odd">
                                                            ...
                                                            <td>
                                                                @if($item->bg_color)
                                                                    <span style="width: 10%;display: inline-block;height: 35px;border: 0;background-color: {{$item->bg_color}};" class="input-group-addon"></span>
                                                                @endif
                                                            </td>
                                                            <td>
                                                                <a href="{{route('add-planning-status',$item->id)}}" class="btn btn-primary product-btn"><i class="fa fa-edit"></i> {{__('text.Edit')}}</a>
                                                                <a href="{{route('delete-planning-status',$item->id)}}" class="btn btn-danger product-btn"><i class="fa fa-trash"></i> {{__('text.Remove')}}</a>
                                                            </td>
                                                        </tr>
                                                    @endforeach
                                                    </tbody>
