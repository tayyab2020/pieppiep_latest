...
class ExportProductsPrestashopOld implements ShouldQueue
{
    ...
    public function create_update_feature($feature,$check_prestashop_feature,$user_id,$related_users,$type,$product_id)
    {
        ...
        if($type == 1)
        {
            $feature_search = $this->feature_search($check_prestashop_feature->prestashop_feature_id,$feature_heading);
        }
        else
        {
            $feature_search = $this->feature_search(NULL,$feature_heading);
        }
        ...
        /*foreach($feature->features as $feature_value)
        {
            ...
            if(!$feature_value->sub_feature && $feature_value->status)
            {
                ...
                $check_features_value = prestashop_features_values_exports::where("retailer_id",$user_id)->where("heading_id",$check_prestashop_feature->id)->where("feature_value_id",$feature_value->id)->first();

                if($check_features_value)
                {
                    $value_id = $check_features_value->prestashop_feature_value_id;
                    $feature_value_search = $this->feature_value_search($value_id,$value,$prestashop_feature_id);
                }
                else
                {
                    $feature_value_search = $this->feature_value_search(NULL,$value,$prestashop_feature_id);
                }
                ...
                $check_features_value->feature_value_id = $feature_value->id;
            ...
            \Log::info('Execution time for features create: ' . $executionTime . ' seconds.');
        }*/
        
        // $unmatched_feature_values = prestashop_features_values_exports::whereNotIn("id",$f_v_ids)->where("heading_id",$check_prestashop_feature->id)->where("retailer_id",$user_id)->get();

        // foreach($unmatched_feature_values as $un)
        // {
        //     $apiUrl = $this->base_url."/api/product_feature_values/".$un->prestashop_feature_value_id;
        //     $ch = curl_init($apiUrl);
        ...
        /*prestashop_features_values_exports::whereNotIn("id",$f_v_ids)->where("heading_id",$check_prestashop_feature->id)->where("retailer_id",$user_id)->delete();

        $export_data = [$prestashop_feature_id,$export_values];

        return [$check_prestashop_feature->id,$export_data]; */
        
        // Batch data for cURL requests
        $apiRequests = [];
        $f_v_ids = [];
        $export_values = [];
        ...
        foreach($feature->features as $feature_value)
        {
            ...
            {
                $value = $feature_value->title;
                $check_features_values[$feature_value->id] = prestashop_features_values_exports::whereIn("retailer_id",$related_users)->where("heading_id",$check_prestashop_feature->id)->where("feature_value_id",$feature_value->id)->first();

                if($check_features_values[$feature_value->id])
                {
                    $value_id = $check_features_values[$feature_value->id]->prestashop_feature_value_id;
                    $feature_value_search = $this->feature_value_search($value_id,$value,$prestashop_feature_id);
                }
                else
                {
                ...
                // Prepare the cURL handle for multi exec
                $ch = curl_init($apiUrl);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_CUSTOMREQUEST, $req_type);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $xmlData);
        ...
        foreach ($handles as $feature_value_id => $handle) {
            $start = microtime(true); // Start time
            ...
            $prestashop_feature_value_id = $response["product_feature_value"]["id"];
            $check_features_value = $check_features_values[$feature_value_id];
    ...
    public function create_attribute_xml($title)
    {
        return <<<XML
        <?xml version="1.0" encoding="UTF-8"?>
        <prestashop xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    public function attributes_search($attribute,$attributes)
    {
        $start = microtime(true); // Start time
        ...
        if(!$attribute_search)
        {
            $xmlData = $this->create_attribute_xml($attribute);
    ...
    public function attribute_value_search($id)
    {
        $url = $this->base_url."/api/product_option_values/".$id;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    ...
    public function feature_search($id,$feature_heading)
    {
        $start = microtime(true); // Start time
        ...
        if($id)
        {
            $url = $this->base_url."/api/product_features/".$id;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        ...
        if(!$response)
        {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            $response_features = curl_exec($ch);
            curl_close($ch);
            $response_features = json_decode($response_features, true);
            ...
            if(isset($response_features["product_features"]))
            {
                ...
                // Initialize cURL handles for each feature request
                ...
                // Fetch responses and remove handles
                foreach ($handles as $f_id => $handle) {
                    $response_feature = json_decode(curl_multi_getcontent($handle), true);
                    $features[] = $response_feature["product_feature"];
                    curl_multi_remove_handle($mh, $handle);
                }
                ...
                $response = $this->recursiveArraySearch($feature_heading, $features);
            }
        }
        else
        {
            $response = $response["product_feature"]["id"];
        }
    ...
    public function feature_value_search($id,$value,$prestashop_feature_id)
    {
        $start = microtime(true); // Start time
        ...
        if($id)
        {
            $url = $this->base_url."/api/product_feature_values/".$id;
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        ...
        if(!$response)
        {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, $this->headers);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            $response_feature_values = curl_exec($ch);
            curl_close($ch);
            $response_feature_values = json_decode($response_feature_values, true);
    
            if(isset($response_feature_values["product_feature_values"]))
            {
                $feature_values_list = $response_feature_values["product_feature_values"];
                $batch_size = 10;  // Set your batch size here
                $total_features = count($feature_values_list);
            
                for ($i = 0; $i < $total_features; $i += $batch_size) {
                    $mh = curl_multi_init();
                    $handles = [];
                    $batch = array_slice($feature_values_list, $i, $batch_size);
            
                    // Initialize cURL handles for each feature value request in the batch
                    foreach ($batch as $feature_value) {
                    ...
                    // Execute all requests concurrently within the batch
                    $running = null;
                    do {
                        ...
                        if ($status > 0) {
                            ...
                            \Log::error("cURL multi_exec error: $error");
                        }
                        curl_multi_select($mh);
                    } while ($running > 0);
            
                    // Fetch responses and remove handles
                    foreach ($handles as $fv_id => $handle) {
                        $raw_response = curl_multi_getcontent($handle);
                        $response_feature_value = json_decode($raw_response, true);
            
                        // Log raw response for debugging
                        \Log::info("Raw response for feature value ID $fv_id: " . $raw_response);
            
                        // Check for JSON decode errors
                        if (json_last_error() !== JSON_ERROR_NONE) {
                            $json_error = json_last_error_msg();
                            \Log::error("JSON decode error for feature value ID $fv_id: $json_error");
                        }
            
                        if ($response_feature_value["product_feature_value"]["id_feature"] == $prestashop_feature_id) {
                            $feature_values[] = $response_feature_value["product_feature_value"];
                        }
                        
                        curl_multi_remove_handle($mh, $handle);
                    }
            
                    // Close the multi-handle for the current batch
                    curl_multi_close($mh);
                }
    
                $response = $this->recursiveArraySearch($value, $feature_values);
            }
        }
        else
        {
            $response = $response["product_feature_value"]["id"];
        }
    ...
    public function create_product_xml($main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$id,$description,$tax_rule_group,$default_image_id)
    {
        if($default_image_id)
        {
    ...
    public function create_update_product($product_export,$main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$user_id,$related_users,$temp,$ncm,$model_ids,$color_ids,$model_prices,$color,$description,$supplier_id,$tax_rule_group,$allImages)
    {
        $default_image_id = "";
        ...
        if($product_export)
        {
            ...
            if($product_search)
            {
                $xmlData = $this->create_product_xml($main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$product_export->prestashop_product_id,$description,$tax_rule_group,NULL);
                $req_type = "PUT";
            }
            else
            {
                $xmlData = $this->create_product_xml($main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,NULL,$description,$tax_rule_group,NULL);
                $req_type = "POST";
            }
        }
        else
        ...
        if(!$ncm)
        {
            ...
            if($default_image_id)
            {
                $xmlData = $this->create_product_xml($main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$product_id,$description,$tax_rule_group,$default_image_id);
                $req_type = "PUT";
                $apiUrl = $this->base_url."/api/products";
                $response = $this->executeCurlRequest($apiUrl, $req_type, $xmlData);
            }
    ...
    public function handle()
    {
        ...
        $this->getAttributes();
        $models_attribute_id = $this->attributes_search("Models",$this->prestashop_attributes);
        $colors_attribute_id = $this->attributes_search("Colors",$this->prestashop_attributes);
        $this->getCategories();
        ...
        foreach($suppliers as $s => $key)
        {
            ...
            $products = Products::where("organization_id",$supplier_organization->id)->where("category_id",$floor_category_id);
                ...
                if(!$temp->deleted_at)
                {
                    ...
                    foreach($combination_based_models as $model)
                    {
                        ...
                        if($check_prestashop_av)
                        {
                            ...
                            {
                                $xmlData = $this->create_attribute_value_xml($model->model,$models_attribute_id,$check_prestashop_av->prestashop_attribute_value_id);
                                $req_type = "PUT";
                            }
                            else
                            {
                                $xmlData = $this->create_attribute_value_xml($model->model,$models_attribute_id);
                                $req_type = "POST";
                            }
                        }
                        else
                        ...
                        foreach($non_combination_based_models as $ncm)
                        {
                            ...
                            if(count($colors))
                            {
                                foreach($colors as $color)
                                {
                                    ...
                                    $product_export = prestashop_products_exports::whereIn("retailer_id",$related_users)->where("product_id",$temp->id)->where("non_combination_model_id",$ncm->id)->where("color_id",$color->id)->first();
                                    $y_array[] = $this->create_update_product($product_export,$main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$user_id,$related_users,$temp,$ncm,$model_ids,$color_ids,$model_prices,$color,$description,$key,$tax_rule_group,$allImages);
                                }
                            }
                            else
                            {
                                ...
                                $product_export = prestashop_products_exports::whereIn("retailer_id",$related_users)->where("product_id",$temp->id)->where("non_combination_model_id",$ncm->id)->where("color_id",NULL)->first();
                                $y_array[] = $this->create_update_product($product_export,$main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$user_id,$related_users,$temp,$ncm,$model_ids,$color_ids,$model_prices,NULL,$description,$key,$tax_rule_group,NULL);
                            }
                        }
                    ...
                    if(count($combination_based_models) || (count($non_combination_based_models) == 0 && count($combination_based_models) == 0))
                    {
                        ...
                        if(count($colors))
                        {
                            foreach($colors as $color)
                            {
                                ...
                                $product_export = prestashop_products_exports::whereIn("retailer_id",$related_users)->where("product_id",$temp->id)->where("non_combination_model_id",NULL)->where("color_id",$color->id)->first();
                                $y_array[] = $this->create_update_product($product_export,$main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$user_id,$related_users,$temp,NULL,$model_ids,$color_ids,$model_prices,$color,$description,$key,$tax_rule_group,$allImages);
                            }
                        }
                        else
                        {
                            ...
                            $product_export = prestashop_products_exports::whereIn("retailer_id",$related_users)->where("product_id",$temp->id)->where("non_combination_model_id",NULL)->where("color_id",NULL)->first();
                            $y_array[] = $this->create_update_product($product_export,$main_category_id,$title,$price,$unit_price,$unit,$categories_xml,$features_xml,$user_id,$related_users,$temp,NULL,$model_ids,$color_ids,$model_prices,NULL,$description,$key,$tax_rule_group,NULL);
                        }
            ...
            $unmatched_products = prestashop_products_exports::whereIn("product_id",$p_array)->whereNotIn("id",$y_array)->where("supplier_id",$key)->whereIn("retailer_id",$related_users)->get();
            $mh = curl_multi_init();
            $handles = [];
            $start = microtime(true); // Start time
            ...
            prestashop_products_exports::whereIn("product_id",$p_array)->whereNotIn("id",$y_array)->where("supplier_id",$key)->whereIn("retailer_id",$related_users)->delete();
