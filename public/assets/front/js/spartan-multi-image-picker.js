        ...
        function addRow(settings, el, edit = 0){
            ...
            if(edit !== 1 && settings.dataImage)
            {
                var template = `<div class="${groupClassName} spartan_item_wrapper" data-spartanindexrow="${count}" style="margin-bottom : 20px; ">`+
                    ...
                    `<a href="javascript:void(0)" data-spartanindexremove="${count}" style="position: absolute !important; right : 3px; top: 3px; background : #ED3C20; border-radius: 3px; width: 30px; height: 30px; line-height : 30px; text-align: center; text-decoration : none; color : #FFF;" class="spartan_remove_row_new"><i class="fa fa-times"></i></a>`+
                ...
                $(html).on("click", ".spartan_remove_row_new", function(){
                    ...
                    $(this).parents(".spartan_item_wrapper").parent().next('input').val("");
                    $(this).parents(".spartan_item_wrapper").parent().next('input').next('input').val(1);

                    settings.dataImage = '';

                    $(this).parents(".spartan_item_wrapper").parent().remove();
