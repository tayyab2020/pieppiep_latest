...
class ExportInvoicesReeleezee implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $url = "https://apps.reeleezee.nl/api/v1/salesinvoices";
        ...
        curl_close($ch);
        $response_invoices = json_decode($response, true);

        if(!isset($response_invoices["value"]))
        {
            $array_statuses[] = $response_invoices["Message"];
        }
        else
        {
            $invoices = all_invoices::whereIn("creator_id",$user_id);

            if($export_by == 1)
            {
                if($created_start_date)
                {
                    $invoices = $invoices->whereDate('document_date', '>=', $created_start_date);
                }

                if($created_end_date)
                {
                    $invoices = $invoices->whereDate('document_date', '<=', $created_end_date);
                }
            }
            else if($export_by == 2)
            {
                ...
                {
                    $invoices = $invoices->whereDate('updated_at', '>=', $updated_start_date);
                }

                if($updated_end_date)
                {
                    $invoices = $invoices->whereDate('updated_at', '<=', $updated_end_date);
                }
            }
            else
            {
                if($last_start_date)
                {
                    $invoices = $invoices->whereDate('document_date', '>=', $last_start_date);
                }
            }

            // $invoices = $invoices->whereDate('created_at', '=', date('Y-m-d'))->withTrashed()->get();
            ...
            foreach($invoices as $our)
            {
                ...
                if(($export_by == 1 || $export_by == 2) || ((!$our->reeleezee_exported_at) || (strtotime($our->updated_at) > strtotime($our->reeleezee_exported_at))))
                {
                    ...
                    $search_by_id_index = array_search($reeleezee_guid, array_column($response_invoices["value"], 'id'));
                    $search_by_inv_index = array_search($invoice_number, array_column($response_invoices["value"], 'Reference'));
        
                    if(is_numeric($search_by_id_index) || is_numeric($search_by_inv_index))
                    {
                        $index = $search_by_id_index ? $search_by_id_index : $search_by_inv_index;
                        $id = $response_invoices["value"][$index]["id"];

                        $url = "https://apps.reeleezee.nl/api/v1/salesinvoices/{$id}";
                        $ch = curl_init();
                        curl_setopt($ch, CURLOPT_URL, $url);
                        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                        $response = curl_exec($ch);
                        curl_close($ch);
                        $response_invoice = json_decode($response, true);

                        $is_vat_included = $response_invoices["value"][$index]["Status"] != 1 && isset($response_invoice["IsVatIncluded"]) ? $response_invoice["IsVatIncluded"] : $vat_settings;
                    }
                    else
                    {
                    ...
                    if($is_deleted)
                    {
                        if(is_numeric($search_by_id_index) || is_numeric($search_by_inv_index))
                        {
                            ...
                            if($response)
                            {
                                $array_statuses[] = $response["Message"] . " Deletion error-> Invoice No: " . $our->invoice_number;
                            }
                        }
                    }
                    else
                    {
                        ...
                        foreach($lines as $i => $line)
                        {
                            ...
                            $data_lines[$i] = array('id' => $line_id, 'Quantity' => $qty, 'Price' => $price, 'Description' => $line->description, 'TaxRate' => $line_tax);
    
                            $data_lines[$i] += array('DiscountAmount' => $line->total_discount * -1);

                            // if($line->discount_option)
                            // {
                            //     $data_lines[$i] += array('DiscountAmount' => $our->negative_invoice ? $line->total_discount * -1 : $line->total_discount);
                            // }
                            // else
                            // {
                            //     $data_lines[$i] += array('DiscountPercentage' => $our->negative_invoice ? ($line->discount/100) * -1 : $line->discount/100);
                            // }
                        ...
                        $data = array('Header' => $header, 'Bottom' => $our_description, 'Footer' => $our_description, 'Description' => $our_description, 'IsVatIncluded' => $is_vat_included, 'Reference' => $invoice_number, 'Entity' => $customer_entity, 'DocumentLineList' => $data_lines, 'BookDate' => $document_date, 'Date' => $document_date, 'Status' => 2, 'Origin' => 2);
                        $data_json = json_encode($data);
                
                        $url = "https://apps.reeleezee.nl/api/v1/salesinvoices/{$id}?$"."expand=*($"."levels=max)";
                        $ch = curl_init();
                        curl_setopt($ch, CURLOPT_URL, $url);
                        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'PUT');
                        ...
                        if($response)
                        {
                            $array_statuses[] = $response["Message"] . " Create/Update error-> Invoice No: " . $our->invoice_number;
                        }
                        else
                        {
                        ...
                        if(!is_numeric($search_by_id_index) && !is_numeric($search_by_inv_index))
                        {
                            ...
                            $url = "https://apps.reeleezee.nl/api/v1/salesinvoices/{$id}/actions";
                            $ch = curl_init();
                            curl_setopt($ch, CURLOPT_URL, $url);
                            curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
                            ...
                            if($response)
                            {
                                $array_statuses[] = $response["Message"] . " Action error-> Invoice No: " . $our->invoice_number;
                            }
                        }
                    }
                }
            ...
            $array_statuses[] = __("text.Invoices exported successfully");
        }
        ...
        \Mail::send(array(), array(), function ($message) use ($user,$msg) {
            $message->to("tayyabkhurram62@gmail.com")
                ...
                ->subject(__("text.Export invoices to reeleezee job status!"))
                ->html($msg, 'text/html');
        });
    }
    ...
    public function failed()
    {
        ...
        $msg = 'Job failed for exporting invoices to Reeleezee <br> Retailer: ' . $user->name .' ('.$user->company_name.')';
