...
<body>
    ...
    function updateModelPrice(element) {
        ...
        row.find('.model-price').text('€ ' + modelPrice.toFixed(2).replace('.', ','));
    }
...
function filterFeaturesBasedOnModel(modelId) {
    ...
    if (model && model.linked_features) {
        ...
        var modelFeatures = model.linked_features.map(f => ({
            id: f.id,
            title: f.title,
            value: f.pivot ? f.pivot.value : null,
            priceImpact: f.pivot ? f.pivot.price_impact : null,
        ...
        $('.feature-select-container').each(function() {
            ...
            featureOptions.each(function() {
                ...
                if (featureData && featureData.linked == 1) {
                    flag = 1;
                    option.show();
                    option.text(featureData.title + (featureData.value ? ` (Value: ${featureData.value})` : ''));
                    // Include price impact if applicable
    ...
    function findBasePrice(tableId, width, height) {
        // Fetch the price table data based on the tableId
        var basePrice = 0;
        var maxBasePrice = 0;
