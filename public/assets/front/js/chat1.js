$(document).ready(function () {
    ...
    $('body').on('click', '.contacts-list .contacts-link', function () {
        $(".main").addClass("main-visible");
    });
    $('body').on('click', '.contacts-list .media-link', function () {
        $(".main").addClass("main-visible");
    })
    ...
    $('#appNavTab .nav-link').on('click', function () {
        ...
        if(jQuery.inArray(current, ["chats-tab","calls-tab","friends-tab","profile-tab"]) == -1)
        {
    ...
    $('.main').on('click', function () {
        ...
        var current = $("#appNavTab").find(".nav-link.active").attr("id");
        if(jQuery.inArray(current, ["chats-tab","calls-tab","friends-tab","profile-tab"]) == -1)
        {
            $("#appNavTab .nav-link").removeClass("active");
            
            if($("body").hasClass('chats-tab-open'))
            {
                $("#chats-tab").addClass("active");
            }
            else if($("body").hasClass('calls-tab-open'))
            {
                $("#calls-tab").addClass("active");
            }
            else if($("body").hasClass('friends-tab-open'))
            {
    ...
    // Todo task done
    $(document).on('click', '.todo-item input[type="checkbox"]', function (event) {
        if ($(this).is(":checked")) {
